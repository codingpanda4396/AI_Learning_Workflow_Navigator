<template>
  <PageContainer>
    <div class="min-h-screen bg-background">
      ...
      <main
        class="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 pb-12 pt-4 md:max-w-4xl md:px-6 lg:px-8"
      >
        <LoadingState
          v-if="loading && !plan"
        ...
        <template v-else-if="plan && decisionModel">
          <PlanDecisionHero
            ...
            @start="onStartStep"
          />
          <PlanReasonBlock :model="decisionModel.reason" />
          <PlanCausalChain :model="decisionModel.causal" />
          <PlanFirstTaskCard :model="decisionModel.firstTask" />
          <PlanPathPreview :model="decisionModel.pathPreview" />
          <PlanContrastBlock :model="decisionModel.contrast" />
        </template>
      </main>
    </div>
  </PageContainer>
...
import PlanCausalChain from '@/components/plan/decision/PlanCausalChain.vue'
...
import PlanContrastBlock from '@/components/plan/decision/PlanContrastBlock.vue'
