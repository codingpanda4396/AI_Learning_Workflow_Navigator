...
<template>
  <SectionCard strong title="策略 + AI问答" description="需要时再展开，不打断你先开始。">
    <button
      type="button"
      class="flex w-full items-center justify-between rounded-[22px] border border-slate-200 bg-slate-50 px-4 py-4 text-left"
      @click="open = !open"
    >
      <span class="text-base font-semibold text-slate-950">可选交互区</span>
    ...
    <div v-if="open" class="mt-4 grid gap-5 lg:grid-cols-2">
      <div class="rounded-[24px] border border-slate-200 bg-white px-5 py-5">
        <p class="text-sm font-semibold text-slate-950">换一种策略</p>
      ...
      <div class="rounded-[24px] border border-slate-200 bg-white px-5 py-5">
        <p class="text-sm font-semibold text-slate-950">AI问一句</p>
