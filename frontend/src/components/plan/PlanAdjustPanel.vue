...
<template>
  <PageSection
    eyebrow="微调设置"
    title="想调整一下的话，可以在这里改"
    description="这里不是主流程，只在你想换学习节奏、方式或起点策略时再调整。"
    compact
  >
    ...
    <div class="mt-5 flex flex-col gap-3 rounded-[1.6rem] border border-dashed border-slate-200 bg-white p-4 md:flex-row md:items-center md:justify-between">
      <p class="text-sm leading-6 text-slate-600">
        {{ regenerating ? '正在根据最新设置重新生成学习规划。' : '如果你想换一种节奏开始，可以先调整设置，再重新生成规划。' }}
      </p>
      <button
        type="button"
        class="rounded-2xl border border-slate-200 bg-slate-50 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60"
        :disabled="disabled"
