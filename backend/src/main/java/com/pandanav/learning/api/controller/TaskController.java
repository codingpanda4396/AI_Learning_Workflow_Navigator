...
public class TaskController {
    ...
    @Operation(summary = "Advance learning step")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "OK"),
        @ApiResponse(responseCode = "404", description = "Not Found",
            content = @Content(schema = @Schema(implementation = ApiErrorResponse.class))),
    ...
    @PostMapping("/{taskId}/steps/{stepId}/advance")
    public AdvanceStepResponse advanceStep(
        @PathVariable @Positive Long taskId,
        @PathVariable @Positive Long stepId,
        @Valid @RequestBody AdvanceStepRequest request
    ) {
        return advanceLearningStepUseCase.advance(taskId, stepId, request);
    }
}
