...
class LlmCallLoggerTest {
    ...
    void shouldWriteStructuredSuccessAndFailureFields() {
        ...
        try {
            ...
            assertTrue(logs.contains("LLM_CALL_SUCCESS stage=CAPABILITY_SUMMARY model=qwen3.5-plus latencyMs=842 promptTokens=231 completionTokens=112 totalTokens=343"));
            assertTrue(logs.contains("LLM_CALL_FALLBACK stage=CAPABILITY_SUMMARY model=qwen3.5-plus reason=LLM_TIMEOUT"));
            assertTrue(logs.contains("LLM_CALL_FAILURE stage=CAPABILITY_SUMMARY model=qwen3.5-plus errorType=JSON_PARSE_ERROR"));
        } finally {
