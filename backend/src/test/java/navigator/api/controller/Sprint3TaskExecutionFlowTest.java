...
class Sprint3TaskExecutionFlowTest {
    ...
    void scaffoldThenMessagesSelfExplainCheckpointComplete() throws Exception {
        ...
        mvc.perform(get("/api/sessions/" + sessionId + "/current-task-guidance").cookie(authCookie))
                .andExpect(status().isOk())
        ...
        mvc.perform(post("/api/tasks/" + taskId + "/self-explanation").cookie(authCookie).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"sessionId\":\"" + sessionId + "\",\"content\":\"我理解链表是通过指针把节点串起来的结构，例如两个节点用 next 连接；与数组相比插入时不需要搬移整块内存。\"}"))
                .andExpect(status().isOk())
